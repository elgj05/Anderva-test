...
const Item = ({item}) => {
  ...
  return (
    <Pressable
      ...
      onPress={() => {
        navRef().dispatch(CommonActions.navigate('Business', {id: item.id}));
      }}>
      <Image style={style.itemImage} source={{uri: item.image}} />
      <View style={style.itemSide}>
        <Text style={style.itemLabel}>{item.name}</Text>
        <Text style={style.itemCat}>{item.category?.name}</Text>
        <Text numberOfLines={2} ellipsizeMode={'tail'} style={style.itemDesc}>
          {item.description}
        </Text>
      </View>
    </Pressable>
